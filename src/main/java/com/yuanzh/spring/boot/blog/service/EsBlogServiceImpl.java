...
import com.yuanzh.spring.boot.blog.repository.es.EsBlogRepository;
import com.yuanzh.spring.boot.blog.vo.TagVO;
...
public class EsBlogServiceImpl implements EsBlogService {
	@Autowired
	private EsBlogRepository esBlogRepository;//spring-data-elasticsearchRepository
	@Autowired
	private ElasticsearchTemplate elasticsearchTemplate;//spring-data-es
	@Autowired
	...
	public Page<EsBlog> listNewestEsBlogs(String keyword, Pageable pageable) throws SearchParseException {
		//通过Spring-data API的Sort实现对es提取出的数据排序
		Page<EsBlog> pages = null;
		Sort sort = new Sort(Direction.DESC, "createTime");//spring-data
	...
	public List<TagVO> listTop30Tags() {
		...
		SearchQuery searchQuery = new NativeSearchQueryBuilder().withQuery(matchAllQuery())
				...
				.addAggregation(terms("tags_result").field("tags")
				.order(Terms.Order.count(false)).size(30)).build();
		/* 关于addAggregation(terms("tags").filed("tags"))见API Terms Aggregation
		 * 用于分组聚合，按"tags"这个字段进行分组
		 */
		// 聚合
		Aggregations aggregations = elasticsearchTemplate.query(searchQuery,
			...
			}//匿名类  spring-data-es API
		});
		/*上述等价于es API
			SearchResponse response = client().prepareSearch("blog").addQuery(searchQuery).execute
										().actionGet();
			Aggregations aggregations = response.getAggregations();
			StringTerms modelTerms = (StringTerms) aggregations.asMap().get("tags_result");
			//StringTerms implementsOrExtends Terms
			List<TagVO> list = new ArrayList<>();
			for(Terms.Bucket entry: modelTerms.getBuckets()){
				//System.out.println(entry.getKey()+":"+entry.getDocCount());
				list.add(new TagVo(entry.getKey().toString(),entry.getDocCount()));
			}
		*/
		/*
			聚合结果
		...
		*/
		//得到tags的value值
		StringTerms modelTerms = (StringTerms) aggregations.asMap().get("tags_result");//es
	...
	public List<User> listTop12Users() {
		...
		List<String> usernamelist = new ArrayList<>();// 存储排序后的username
